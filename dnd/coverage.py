...
MONSTER_RACIAL_TRAITS: dict[str, Entry] = {
    ...
    "tail_spikes":           (IMPLEMENTED,    "Manticore 'spikes' is a ranged attack with range_increment=180. 24/day cap is wired via Combatant.daily_resources['tail_spikes'] (set during construction from _apply_monster_racial_traits). _do_attack decrements on each spikes attack and skips when the pool is empty. Daily replenishment happens in tick_round when 14400+ rounds have elapsed since the last refresh — leveraging the global 6-second tick for day cycles. The 6-spike-volley-as-standard-action structure is not separately modeled (the cap counts individual spikes)."),
    ...
    "engulf":                (IMPLEMENTED,    "Cube's end-of-turn rider scans candidates: anyone with engulfed_by_id == cube.id (off-grid) plus any adjacent enemy. Newly engulfed: paralyzed, immediate 1d6 acid, removed from grid (pull-along: cube can move freely; victim is logically inside). Already engulfed: another Reflex save vs DC 10 + 1/2 HD + Str mod. On fail, +1d6 acid; on pass, victim is freed (paralyzed cleared, placed on a free square adjacent to the cube via _place_at_free_adjacent). Move-into-square is approximated by end-of-turn adjacency."),
...
FEATS: dict[str, Entry] = {
    ...
    "improved_grapple":      (IMPLEMENTED,    "no AoO when attempting grapple; +2 CMB on grapple via _resolve_maneuver. CMD-side defensive bonus is not applied yet (deferred — would need a qualifier-based +2 CMD vs grapple)."),
