...
CONDITIONS: dict[str, Entry] = {
    ...
    "cowering":        (PARTIAL,        "denies Dex via _DEX_DENIED_CONDITIONS; validate_turn blocks all action slots. GAP: RAW also imposes -2 AC (not the dodge/Dex denial) — that explicit penalty is not added as a modifier."),
    ...
    "fascinated":      (PARTIAL,        "-4 untyped to skill:perception via _on_condition_applied; validate_turn blocks all action slots. GAP: RAW says any obvious threat (an enemy attacking, a hostile spell, etc.) automatically breaks the effect; the engine does not auto-clear fascinated on threat exposure."),
    "fatigued":        (IMPLEMENTED,    "-2 untyped to ability:str/dex via condition hook; cleared by remove_condition. validate_turn explicitly bans run / charge full-round actions while fatigued (or exhausted, which inherits the ban)."),
    ...
    "panicked":        (PARTIAL,        "-2 morale chain like frightened (tier suppression); validate_turn additionally bans attack/cast/charge/full_attack/cleave/stunning_fist/smite_evil/ready_brace/rage_start/trample/grapple_*. GAP: RAW says a panicked creature drops items it's holding and must flee away from the source of fear; neither drop-on-panic nor flee compulsion is modeled."),
    ...
    "petrified":       (IMPLEMENTED,    "Denies Dex (sneak attack qualifies; petrified is in _DEX_DENIED_CONDITIONS). Helpless follow-on cascade. Turn validation blocks physical acts. Stone-body transformation: on apply, max_hp halves and current_hp is clamped (pre-petrified values stashed in resources for restoration on remove). Take_damage applies hardness 8 reduction and shatters (sets HP below death threshold + dead) on any single hit dealing 50+ damage."),
...
CORE_MECHANICS: dict[str, Entry] = {
    ...
    "combat.5_foot_step":            (PARTIAL,        "validate_turn enforces exclusivity (no movement-move + 5-ft step in the same turn); one square only; does not provoke AoO. GAP: RAW says you can't take a 5-ft step in difficult terrain — _do_5ft_step calls grid.move with no terrain check."),
