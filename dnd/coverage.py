...
CORE_MECHANICS: dict[str, Entry] = {
    ...
    "combat.ranged_touch_attack":    (PARTIAL,        "Weapon ranged-touch attacks use target.touch AC via resolve_attack and apply the -2/range-increment penalty normally. GAP: spell-driven ranged-touch effects (e.g. ray_of_frost, scorching_ray, acid_splash) flagged via effect.ranged_touch_attack=True don't actually roll an attack vs touch AC — they just resolve damage/effect (saving throw if any). The to-hit roll + range increment are both missing on the spell side; needs a refactor in the spell handlers to route the attack through resolve_attack with a touch-AC defense profile."),
