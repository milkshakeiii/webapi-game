...
MONSTER_RACIAL_TRAITS: dict[str, Entry] = {
    ...
    "rock_catching":         (IMPLEMENTED,    "_do_attack post-resolve fires a Reflex save vs the size-scaled DC (small=15 / medium=20 / large=25 / huge=30 / gargantuan=35 / colossal=40) keyed off the THROWER's size. Success converts the hit to a miss and stamps target.resources['last_rock_caught_round'] so a second rock in the same round skips the save entirely."),
    ...
    "tail_spikes":           (IMPLEMENTED,    "Manticore 'spikes' is a ranged attack with range_increment=180. 24/day cap wired via Combatant.daily_resources['tail_spikes']; _do_attack decrements per spike. tick_round replenishes after 14400 rounds (1 day at the 6-second tick). 'tail_spike_volley' composite is the RAW standard-action variant — fires up to 4 spike attacks at one target, stopping early if the daily pool runs dry."),
    ...
    "web_giant_spider":      (IMPLEMENTED,    "'web' composite: 50-ft Reflex save vs DC 10 + 1/2 HD + Con mod or entangled (sourced 'web:<actor.id>'). 8/day cap via Combatant.daily_resources['web_uses'] (replenished daily by tick_round at the 6-second tick). 'escape_web' composite (standard action): max(Str check, Escape Artist roll) vs DC 16; success removes entangled."),
    ...
    "quickness":             (IMPLEMENTED,    "_apply_monster_racial_traits adds +1 enhancement to initiative. _movement_squares() in turn_executor adds +2 squares (+10 ft) when encounter.round_number == 1 and the actor has the trait — applied uniformly to move / charge / withdraw / run."),
    ...
    "engulf":                (PARTIAL,        "Cube's end-of-turn rider scans candidates (engulfed-by-cube + adjacent enemies). Newly engulfed: paralyzed + 1d6 acid + removed from grid. Already engulfed: another Reflex save; fail → +1d6 acid, pass → freed and placed adjacent. GAP: the RAW move-into-square trigger (save fires when the cube moves through the target's square) is approximated as end-of-turn adjacency — a target standing still next to the cube gets engulfed without the cube actually moving."),
...
CLASS_FEATURES_L1: dict[str, Entry] = {
    ...
    "domains":                (IMPLEMENTED,    "12 CRB domains in dnd/content/domains/. Cleric picks 2 via class_choices.domains; each domain's L1 granted power gets a 3+Wis/day pool. composite 'domain_power' dispatches by power kind (touch_heal / touch_buff / touch_info / ranged_damage / touch_condition / self_buff / touch_damage_bleed / ranged_weapon_attack). Domain spells we have in catalog are auto-folded into Combatant.castable_spells + Combatant.domain_spells (per-level set), and one is auto-prepared per level. Bonus domain_slot_<L> = 1 per spell level the cleric can cast, and _do_cast prefers the bonus slot when casting a domain spell at base level (no metamagic); falls back to the regular spell_slot_<L> when the bonus is exhausted."),
    "spontaneous_casting":    (IMPLEMENTED,    "Cleric cure-swap: args.spontaneous_cure=True on a 'cast' lets the actor cast a cure_X spell at any level they have a prepared slot for, burning the first prepared spell at that level in place of the cure. (The druid summon-nature's-ally analogue is a SEPARATE feature, not part of this entry.)"),
    ...
    # Druid
    "nature_bond":            (PARTIAL,         "Choice made at L1 via class_choices.nature_bond_type ∈ {domain, animal_companion} (validated in characters.validate_druid_nature_bond; defaults to animal_companion when omitted, which then requires an animal_companion_base_animal). Domain path picks exactly one of the 7 CRB druid-allowed domains (Air, Animal, Earth, Fire, Plant, Water, Weather). Engine reuses the cleric-domain pipeline: domain L1 power's uses-per-day pool seeded (Icicle, Storm Burst) or skipped if the JSON marks ``deferred_handler`` (Speak with Animals, Wooden Fist); bonus ``domain_slot_<L>`` seeded for every spell level where the domain's spell exists in content; one domain spell per accessible level auto-prepared. The 4 missing CRB domains (Animal, Plant, Water, Weather) added as content. GAPS: animal-companion branch Combatant materialization deferred to slice 3 (validation accepts the pick, but no creature is spawned yet); Speak with Animals and Wooden Fist L1 power handlers not implemented (marked deferred_handler in content); RAW restriction 'this domain slot cannot be used to cast a spell spontaneously' not yet enforced in spell prep; many of the L1 druid-allowed domain spells are missing from content (obscuring_mist, calm_animals, entangle, fog_cloud) so their bonus slots silently no-op at low levels."),
    ...
    "flurry_of_blows":        (IMPLEMENTED,     "full_attack composite accepts options.flurry: True. When set on a monk wielding any weapon in the monk-special-weapon group (_MONK_FLURRY_WEAPONS: unarmed_strike, kama, nunchaku, quarterstaff, sai, shuriken, siangham, temple_sword), _do_full_attack swaps in monk-level as the BAB-from-monk-class-levels (RAW per-attack delta = monk_level - actor_bab), adds +1/+2/+3 extra attacks at L1/8/15, and applies -2 to all attack rolls (TWF-like). Full Strength on damage. options.flurry_substitutions: list of per-position maneuver kinds (None | trip | disarm | sunder) lets the patron substitute combat maneuvers for unarmed attacks; CMB roll inherits the flurry's BAB swap-in and -2 penalty via _resolve_maneuver(cmb_delta=...). Substitution kinds outside the RAW menu (grapple, bull_rush, etc.) are rejected with a skip event. Natural-attack chain is suppressed while flurry is active (RAW)."),
    ...
    "detect_evil":            (PARTIAL,        "Composite 'detect_evil' picks a target and emits an event with target.alignment + is_evil flag. GAP: RAW behavior is a 60-ft cone scan with concentration over up to 3 rounds (round 1: presence, round 2: number of auras, round 3: locations + power) — only the single-target reveal works today, which is enough for AI smite-targeting but not for the RAW area scan."),
    ...
    # Sorcerer
    "bloodline":              (PARTIAL,         "Choice made at L1 via class_choices.sorcerer_bloodline (validated in characters.validate_sorcerer_bloodline_choices). Five CRB bloodlines in content/sorcerer_bloodlines/{arcane,celestial,draconic,fey,infernal}.json. Each bloodline's class skill is added to class_skills at sheet build (Heal / Perception / Knowledge nature / Diplomacy / Knowledge arcana). Draconic also requires class_choices.sorcerer_dragon_type (one of the 10 chromatic/metallic dragons). L1 active power uses/day = 3 + Cha mod, seeded as sorcerer_bloodline_<pid>_uses on Combatant.resources. Active power dispatch implemented as ``sorcerer_bloodline_power`` composite + SorcererBloodlinePower substrate Action; per-power handlers in _BLOODLINE_POWER_HANDLERS: heavenly_fire (alignment-keyed damage/heal, ranged touch 30 ft), draconic_claws (free action; installs 2 claw natural attacks at full BAB; size-scaled dice), laughing_touch (melee touch; applies the new ``laughing`` condition for 1 round, recognized in actions._validate_intent), corrupting_touch (melee touch; shaken for max(1, sorc_lvl // 2) rounds; multiple touches add to duration per RAW). Bloodline arcana wired in spells._bloodline_dc_bonus and spells._draconic_damage_bonus_per_die: fey +2 DC on compulsion subschool, infernal +2 DC on charm subschool, arcane +1 DC on slot-raising metamagic, draconic +1 damage per die rolled on matching-energy spells (matched against dragon_energy_by_type table). GAPS: celestial summon DR/evil — no summoning spells in content yet, helper not wired; arcane bloodline's L1 Arcane Bond power deferred to Phase 4.3d; higher-level powers (L3+), bonus spells (start at L3), and bonus feats (start at L7) all deferred; per-target 24h immunity to laughing touch not modeled (re-applies fire); claws don't tick the rounds/day budget across turns (each invocation consumes one use; RAW says rounds while active)."),
    ...
    # Wizard
    "arcane_school":          (PARTIAL,         "Choice made at L1 via class_choices.wizard_school + class_choices.wizard_opposition_schools, validated in characters.validate_wizard_school_choices (specialists need exactly two distinct opposition schools differing from the specialty; universalists cannot have opposition). Content lives in content/wizard_schools/{abjuration,conjuration,divination,enchantment,evocation,illusion,necromancy,transmutation,universalist}.json. Specialist wizards get +1 spell slot of every spell level they can cast (combatant.py). Opposition-school spell prep now consumes 2 slots per RAW: each prepared spell whose school matches an opposition school deducts one extra from spell_slot_<L>. L1 passive powers applied at sheet build: Forewarned (init), Enchanting Smile (social skills), Physical Enhancement (one physical ability), Resistance (energy resistance 5), Intense Spells (in spells._intense_spells_bonus, applied once per evocation damage roll). L1 active powers (Acid Dart, Telekinetic Fist, Force Missile, Blinding Ray, Hand of the Apprentice, Diviner's Fortune, Dazing Touch, Grave Touch, Protective Ward) implemented as the wizard_school_power composite + WizardSchoolPower substrate Action; uses-per-day = 3 + Int mod gated by wizard_school_<pid>_uses. GAPS: Summoner's Charm (conjuration) and Extended Illusions (illusion) need spell-handler hooks not yet wired; Power Over Undead (necromancy) bonus feat-grant not yet wired; the RAW -4 craft penalty on items requiring opposition-school spells as prerequisites isn't modeled (we don't model item crafting yet)."),
    "arcane_bond":             (PARTIAL,         "Choice made at L1 via class_choices.arcane_bond_type ∈ {amulet, ring, staff, wand, weapon, familiar} (validated in characters.validate_wizard_arcane_bond; defaults to 'amulet' when omitted). Bonded-object forms (everything except familiar) seed an ``arcane_bond_uses: 1`` daily resource. Cast-time hook in _do_cast: when ``use_arcane_bond: True`` is in the cast args, the spell can be drawn from castable_spells even if not in prepared_spells; the pool decrements by 1; metamagic-rejected (RAW 'cannot be modified by metamagic feats'); opposition-school-rejected (RAW 'cannot be used to cast spells from the wizard's opposition schools'). GAPS: familiar bond not yet wired (waits for animal-companion / familiar infrastructure — slice 3); the concentration-check-when-not-holding RAW rule isn't modeled (we always treat the bonded object as available); replacing a lost bonded object (1-week ritual, 200 gp/level) not modeled (no item-loss model); 'wizard can add magic-item creation enchantments to bonded object' deferred (no item-crafting model)."),
...
CONDITIONS: dict[str, Entry] = {
    ...
    "cowering":        (PARTIAL,        "denies Dex via _DEX_DENIED_CONDITIONS; intent-legality check blocks all action slots. GAP: RAW also imposes -2 AC (not the dodge/Dex denial) — that explicit penalty is not added as a modifier."),
    "dazed":           (IMPLEMENTED,    "intent-legality check blocks all action slots (full_round/standard/move/swift/5ft/free)"),
    ...
    "dead":            (IMPLEMENTED,     "set by _apply_post_damage_state; intent-legality check prevents acts"),
    "disabled":        (IMPLEMENTED,    "intent-legality check restricts to one move OR standard (no full-round); run_intent_via_substrate applies 1 HP self-damage after the standard resolves (drops the actor into dying)"),
    ...
    "grappled":        (PARTIAL,         "-2 untyped attack penalty + -4 untyped Dex penalty added on apply; intent-legality check blocks moves, 5-ft step, charge/withdraw/run. GAP: humanoid creatures without two free hands attempting to grapple a foe should take an additional -4 CMB penalty per RAW; the free-hand check is not modeled. Full grapple action set (escape, reverse, etc.) not yet modeled either, but the action restriction is in place."),
    ...
    "laughing":        (IMPLEMENTED,    "Applied by the Fey Bloodline's Laughing Touch (Phase 4.3c). Intent-legality check forbids standard / full-round / swift / 5-ft-step / free while laughing; only a single move action allowed (RAW: 'A laughing creature can only take a move action but can defend itself normally'). Per-target 24-hour immunity after the effect ends not yet modeled — multiple touches in one day will re-laugh the target."),
    "invisible":       (IMPLEMENTED,    "invisible condition sets concealment=50 (and restores prior value on remove). In _do_attack: invisible attacker → +2 melee attack + target denied Dex (flat-footed AC) UNLESS the target previously pinpointed this attacker (via Combatant.pinpointed_invisible). After every attack from an invisible foe, the target rolls a Perception check vs DC 20 + 1/10ft and on success records the attacker_id; that pinpoint persists for the current round and removes the Dex-denied benefit on subsequent attacks. The +2 attack bonus persists either way (RAW: the attack itself is unexpected)."),
    "nauseated":       (IMPLEMENTED,    "intent-legality check allows only a single move action; standard/full_round/swift/5ft/free all banned"),
    "panicked":        (PARTIAL,        "-2 morale chain like frightened (tier suppression); intent-legality check additionally bans attack/cast/charge/full_attack/cleave/stunning_fist/smite_evil/ready_brace/rage_start/trample/grapple_*. GAP: RAW says a panicked creature drops items it's holding and must flee away from the source of fear; neither drop-on-panic nor flee compulsion is modeled."),
    "paralyzed":       (IMPLEMENTED,    "denies Dex (sneak attack qualifies); intent-legality check prevents acts; helpless follow-on wired via _IMPLIES_HELPLESS cascade"),
    "petrified":       (IMPLEMENTED,    "Denies Dex (sneak attack qualifies; petrified is in _DEX_DENIED_CONDITIONS). Helpless follow-on cascade. Intent-legality check blocks physical acts. Stone-body transformation: on apply, max_hp halves and current_hp is clamped (pre-petrified values stashed in resources for restoration on remove). Take_damage applies hardness 8 reduction and shatters (sets HP below death threshold + dead) on any single hit dealing 50+ damage."),
    "pinned":          (PARTIAL,        "denies Dex (sneak attack qualifies); set by grapple_pin composite (regular CMB vs CMD per RAW); also applies 'helpless'. Cleared on grapple_break_free or remove_condition('grappled'). GAP: RAW imposes an explicit -4 AC penalty on pinned creatures (in addition to the Dex denial); this modifier is not added."),
    ...
    "staggered":       (IMPLEMENTED,    "intent-legality check enforces a single move OR standard (no full-round); ferocity sets it"),
    "stunned":         (PARTIAL,        "denies Dex (sneak attack qualifies); cleared automatically via stunned_until_round in tick_round. GAPS: (a) the action ban is enforced only by the stunned_until_round rider — externally-applied stunned (no rider) does NOT block actions in the intent-legality check; (b) RAW says stunned creatures drop everything held — drop-on-stun is not implemented; (c) RAW imposes -2 AC explicitly — not added as a modifier; (d) RAW gives attackers +4 to combat-maneuver checks against a stunned target — not modeled."),
    "unconscious":     (IMPLEMENTED,    "is_unconscious checks the condition; intent-legality check prevents acts; applying 'unconscious' adds 'helpless' (tracked under 'implied_by_unconscious' source)"),
...
FEATS: dict[str, Entry] = {
    ...
    "improved_grapple":      (IMPLEMENTED,    "Offensive: no AoO + 2 CMB on grapple via _resolve_maneuver. Defensive: +2 CMD vs grapple via feat_effects.feat_modifiers (qualifier-based, only applies when context['maneuver']=='grapple')."),
    "improved_disarm":       (IMPLEMENTED,    "Offensive: no AoO + 2 CMB on disarm via _resolve_maneuver. Defensive: +2 CMD vs disarm via feat_effects."),
    "improved_trip":         (IMPLEMENTED,    "Offensive: no AoO + 2 CMB on trip via _resolve_maneuver. Defensive: +2 CMD vs trip via feat_effects."),
    "improved_sunder":       (IMPLEMENTED,    "Offensive: no AoO + 2 CMB on sunder via _resolve_maneuver. Defensive: +2 CMD vs sunder via feat_effects."),
    "improved_bull_rush":    (IMPLEMENTED,    "Offensive: no AoO + 2 CMB on bull_rush via _resolve_maneuver. Defensive: +2 CMD vs bull_rush via feat_effects."),
    "improved_overrun":      (IMPLEMENTED,    "Offensive: no AoO + 2 CMB on overrun via _resolve_maneuver. Defensive: +2 CMD vs overrun via feat_effects."),
    "improved_drag":         (IMPLEMENTED,    "Offensive: no AoO + 2 CMB on drag via _resolve_maneuver. Defensive: +2 CMD vs drag via feat_effects."),
    "improved_reposition":   (IMPLEMENTED,    "Offensive: no AoO + 2 CMB on reposition via _resolve_maneuver. Defensive: +2 CMD vs reposition via feat_effects."),
    "improved_steal":        (IMPLEMENTED,    "Offensive: no AoO + 2 CMB on steal via _resolve_maneuver. Defensive: +2 CMD vs steal via feat_effects."),
    "improved_dirty_trick":  (IMPLEMENTED,    "Offensive: no AoO + 2 CMB on dirty_trick via _resolve_maneuver. Defensive: +2 CMD vs dirty_trick via feat_effects."),
...
CORE_MECHANICS: dict[str, Entry] = {
    ...
    "combat.action_types":           (IMPLEMENTED,    "v1 sugar covers standard/move/swift/free/full_round/5ft_step. run_intent_via_substrate calls actions._validate_intent on the do dict before translating to substrate Actions; illegal combinations skip with reason='invalid_turn'. Substrate-native pickers see the same gating through enumerate_legal_actions / TurnSlots accounting."),
    "combat.5_foot_step":            (PARTIAL,        "intent-legality check enforces exclusivity (no movement-move + 5-ft step in the same turn); substrate's TurnSlots tracking enforces it for picker-driven actors. One square only; does not provoke AoO. GAP: RAW says you can't take a 5-ft step in difficult terrain — _do_5ft_step calls grid.move with no terrain check."),
    "combat.full_round_action":      (IMPLEMENTED,    "intent-legality check forbids full_round + standard/move; charge/withdraw/full_attack are full-round composites"),
    "combat.swift_action":           (IMPLEMENTED,    "swift slot honored — quicken-cast routes through it; smite_evil / rage_start / bardic_performance work via the composite path with the intent-legality check gating them. PF1 RAW limit of one swift per turn is enforced structurally (the do dict has a single ``swift`` key)"),
    ...
    "combat.ranged_touch_attack":    (IMPLEMENTED,    "Weapon ranged-touch attacks use target.touch AC via resolve_attack with the standard -2/range-increment penalty. Spell-driven touch attacks: spells flagged effect.ranged_touch_attack=True (acid_splash, ray_of_frost, disrupt_undead, acid_arrow, searing_light, scorching_ray) or melee_touch_attack=True (touch_of_fatigue, shocking_grasp) now roll a d20 + BAB + Dex/Str + size + general attack mods vs touch AC via resolve_spell_touch_attack. Nat 1 always misses; nat 20 always hits. On miss, the spell has no effect (no damage, no save). Per-ray spells (scorching_ray) roll one touch attack per ray. Magic Missile remains auto-hit (no flag → no roll)."),
    ...
    "combat.combat_maneuver_basic":  (PARTIAL,        "_resolve_maneuver: d20 + actor.cmb vs target.cmd(context={'maneuver': kind}); nat 1 always fails / nat 20 always succeeds per RAW; Stability and other qualified CMD bonuses honored. GAPS: (a) Tiny-or-smaller creatures should use Dex (not Str) for CMB — not modeled; (b) auto-success vs immobilized/unconscious/helpless targets — not modeled; (c) +4 to attacker's CMB vs stunned targets — not modeled."),
    "combat.bull_rush":              (PARTIAL,        "composite 'bull_rush'; pushes target 1 + (margin//5) squares directly away from actor; stops on impassable. GAPS: (a) RAW: bull rush only works on a target no more than one size category larger — size restriction not enforced; (b) RAW lets the actor optionally move with the pushed target (uses available movement) — not modeled."),
    "combat.disarm":                 (PARTIAL,        "composite 'disarm' actually removes the InventoryItem from target.held_items['main_hand']. Margin ≥ 10 transfers it to the attacker's carried_items; otherwise dropped at the target's square (recorded on encounter.dropped_items if available). Attack options pruned so target can't keep using the lost weapon. GAPS: (a) RAW: failing the disarm by 10 or more drops the attacker's own weapon — not modeled; (b) RAW: -4 penalty when disarming while unarmed — not modeled."),
    "combat.drag":                   (IMPLEMENTED,    "composite 'drag': pulls target 1+(margin//5) squares toward actor; stops at actor's square"),
    "combat.grapple":                (IMPLEMENTED,    "composite 'grapple' marks both as 'grappled' and links via grappling_target_id / grappled_by_id. Full action set: grapple_damage (CMB vs CMD → weapon damage), grapple_move (move both up to half speed), grapple_pin (CMB vs CMD → 'pinned' + 'helpless'; RAW: pin is a regular maintain check, no +5 DC), grapple_break_free (CMB or Escape Artist vs grappler's CMB → both lose grappled, target loses pinned/helpless)"),
    "combat.overrun":                (PARTIAL,        "composite 'overrun': actor moves past target; target prone if margin >= 5. GAPS: (a) RAW: target may choose to avoid the overrun (let the actor pass through without a check) — choice not offered; (b) +2 DC per extra leg above two — not modeled."),
    ...
    "combat.dirty_trick":            (IMPLEMENTED,    "composite 'dirty_trick' applies one of {blinded, dazzled, deafened, entangled, shaken, sickened} (chosen via args.condition; default dazzled). Duration = 1 + margin//5 rounds. Provokes AoO unless improved_dirty_trick. The 'standard action to remove' counter-action isn't separately wired."),
    "combat.maneuver_aoo":           (IMPLEMENTED,    "_do_combat_maneuver fires a target AoO before the CMB roll unless the actor has the matching improved_<kind> feat. Skips the AoO when the target is helpless / unconscious / paralyzed / etc. (CANNOT_AOO style filter inline)."),
    ...
    "combat.trip":                   (PARTIAL,        "composite 'trip' applies prone on success; also fires automatically after a successful melee hit for creatures with the 'trip_attack' racial trait (wolf). GAPS: (a) RAW: failing a trip by 10 or more knocks the attacker prone instead — failure-effect path is unreachable since _apply_maneuver_effect runs only on success; (b) +2 DC per leg above two — not modeled; (c) flying / legless / ooze creatures cannot be tripped — immunity not enforced."),
    "combat.coup_de_grace":          (IMPLEMENTED,    "_do_coup_de_grace composite: full-round vs an adjacent helpless target; deals weapon damage at the crit multiplier; target rolls Fort DC 10 + damage or dies. Provokes AoOs from threateners adjacent to the actor (skipping any threatener in the CANNOT_AOO set: helpless / sleeping / paralyzed / petrified / etc.); the actor aborts the strike if dropped to 0 HP by an AoO."),
    ...
    "combat.evasion":                (NOT_IMPLEMENTED, "Rogue/monk class feature (L2 rogue, L2 monk): a successful Reflex save against an effect that normally deals half damage on save instead deals zero damage. Engine halves regardless of class feature; no evasion check."),
    "combat.improved_evasion":       (NOT_IMPLEMENTED, "Rogue advanced talent / monk L9 class feature: success deals zero, FAIL deals half. Builds on evasion."),
    "combat.mettle":                 (NOT_IMPLEMENTED, "Inquisitor / similar class feature: like evasion but for Fortitude / Will saves on partial-effect spells (e.g., 'Fort partial' or 'Will partial'). Not implemented; partial-save semantics (negates lesser condition on success) are honored only via the spell's condition_on_save_success field."),
    "combat.broken_item_penalties":  (NOT_IMPLEMENTED, "InventoryItem.broken flag is set when an item drops to half HP (in inventory.py) but is never consulted during attack/AC computation. RAW penalties unmodeled: broken weapon -2 attack and damage; crits only on natural 20 with ×2 multiplier; broken armor/shield bonus halved (round down) and double the armor check penalty; broken wand/staff uses charges twice as fast."),
    ...
    "combat.mounted_combat":         (PARTIAL,        "mount_id/rider_id link; mount/dismount composite actions; rider position follows mount in _move_along; mounted-lance charge ×2 damage (×3 with Spirited Charge feat); Mounted Combat feat lets rider negate hit-on-mount via Ride check (DC = attack roll). GAPS: Trample / Ride-By Attack each have their own feat coverage entries with their own gaps. Mount autonomous AI on the rider's turn isn't simulated — the mount only acts when the rider's script directs it."),
    ...
    "magic.spell_resistance":        (PARTIAL,        "d20 + caster_level vs target SR via spells.overcomes_sr; gated on spell.spell_resistance == 'yes'. GAP: SR is read only off monster templates (target.template_kind == 'monster'); characters with SR (drow racial SR, SR-granting items, Spell Resistance buff spell) are silently treated as SR 0."),
    ...
    "magic.concentration_entangled": (NOT_IMPLEMENTED, "RAW: an entangled caster needs a DC 15 + spell level concentration check to cast. Not modeled — only grappled-style concentration is wired."),
    "magic.concentration_pinned":    (NOT_IMPLEMENTED, "RAW: pinned creatures cannot use somatic-component spells, and grappled-style concentration triggers should still apply if at all possible. Pinned-vs-S-component is not enforced."),
    "magic.concentration_motion":    (NOT_IMPLEMENTED, "RAW: vigorous motion (DC 10 + spell level), violent motion (DC 15 + spell level), severe weather, etc. all trigger concentration checks. None of these are modeled."),
