...
MONSTER_RACIAL_TRAITS: dict[str, Entry] = {
    ...
    "rock_catching":         (PARTIAL,        "_do_attack post-resolve fires a Reflex DC 15 for the target on a thrown 'rock' hit; success converts to miss. GAPS: size-based DC (15/20/25 by rock size) is simplified to a flat 15, and the RAW 'once per round' limit isn't tracked."),
    ...
    "tail_spikes":           (PARTIAL,        "Manticore 'spikes' is a ranged attack with range_increment=180. 24/day cap wired via Combatant.daily_resources['tail_spikes']; _do_attack decrements per spike and skips when empty; tick_round replenishes after 14400 rounds (1 day at the 6-second tick). GAP: the RAW '6-spike volley as a standard action' structure isn't a separate composite — the cap just counts individual spikes."),
    ...
    "web_giant_spider":      (PARTIAL,        "'web' composite: 50-ft Reflex save vs DC 10 + 1/2 HD + Con mod or entangled (sourced 'web:<actor.id>'). GAPS: the 8/day cap isn't enforced, and the RAW Strength check / Escape Artist DC 16 to break free from a web isn't a separate action — the entangled condition just sticks until removed externally."),
...
CONDITIONS: dict[str, Entry] = {
    ...
    "fatigued":        (PARTIAL,         "-2 untyped to ability:str/dex via condition hook on add_condition; cleared by remove_condition. GAP: the RAW 'cannot run or charge while fatigued' ban is not enforced explicitly in validate_turn — a fatigued creature with high enough Str/Dex could still run/charge in our system, which is wrong."),
    ...
    "invisible":       (PARTIAL,        "invisible condition sets concealment=50 (and restores prior value on remove). In _do_attack: invisible attacker → +2 melee attack + target denied Dex (flat-footed AC). GAP: pinpoint-by-Perception isn't modeled — attackers in our engine always treat invisible targets as undetected. RAW lets a creature roll a Perception check (DC 20 + extra) to pinpoint an invisible foe and remove the +2/Dex-denied penalties on subsequent attacks."),
    ...
    "petrified":       (PARTIAL,        "denies Dex (sneak attack qualifies — petrified added to _DEX_DENIED_CONDITIONS); turn validation prevents physical acts; helpless follow-on wired via _IMPLIES_HELPLESS cascade. GAP: the RAW stone-body transformation (hardness 8, half remaining HP, destroyed if shattered for 50+ damage) isn't modeled — a petrified creature still has its original HP/AC."),
...
FEATS: dict[str, Entry] = {
    ...
    "improved_grapple":      (PARTIAL,        "Offensive half wired: no AoO when attempting grapple; +2 CMB on grapple via _resolve_maneuver. GAP: defensive half (+2 CMD when someone grapples you) is not applied yet — would need a qualifier-based +2 CMD vs grapple, which is straightforward to add but not yet in racial_effects/feat_effects."),
    "improved_disarm":       (IMPLEMENTED,    "no AoO when attempting disarm; +2 CMB on disarm via _resolve_maneuver"),
    "improved_trip":         (IMPLEMENTED,    "no AoO when attempting trip; +2 CMB on trip via _resolve_maneuver"),
    "improved_sunder":       (IMPLEMENTED,    "no AoO when attempting sunder; +2 CMB on sunder via _resolve_maneuver"),
    "improved_bull_rush":    (IMPLEMENTED,    "no AoO when attempting bull rush; +2 CMB on bull_rush via _resolve_maneuver"),
    "improved_overrun":      (IMPLEMENTED,    "no AoO when attempting overrun; +2 CMB on overrun via _resolve_maneuver"),
    "improved_drag":         (IMPLEMENTED,    "no AoO when attempting drag; +2 CMB on drag via _resolve_maneuver"),
    "improved_reposition":   (IMPLEMENTED,    "no AoO when attempting reposition; +2 CMB on reposition via _resolve_maneuver"),
    "improved_steal":        (IMPLEMENTED,    "no AoO when attempting steal; +2 CMB on steal via _resolve_maneuver"),
    "improved_dirty_trick":  (IMPLEMENTED,    "no AoO when attempting dirty_trick; +2 CMB on dirty_trick via _resolve_maneuver"),
...
CORE_MECHANICS: dict[str, Entry] = {
    ...
    "combat.ranged_touch_attack":    (PARTIAL,        "Touch AC pathway works (chosen attack option's resolve_attack uses target.touch AC). GAP: range-increment penalty for ranged touch attacks (-2 per increment past the first) isn't enforced — long-distance ray spells don't degrade in accuracy."),
