...
CONDITIONS: dict[str, Entry] = {
    ...
    "incorporeal":     (PARTIAL,        "incorporeal-subtyped monsters auto-receive concealment=50 at construction (50% miss for attackers). NOT YET MODELED: corporeal-source damage is not halved; the engine doesn't tag attacks as force / ghost-touch (so the 'except force/ghost-touch' bypass is also missing); incorporeal immunity to non-magical attacks isn't enforced. All three gaps need an attack-tag system to land — tracked TODO in combatant_from_monster."),
